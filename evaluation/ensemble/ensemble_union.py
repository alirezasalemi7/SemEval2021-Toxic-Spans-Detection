...
with open("spans-pred-bert.txt") as bert,open("spans-pred-electra.txt") as electra,open("spans-pred-roberta.txt") as roberta, open("ensemble_union.txt","w") as output:
    i = 0
    while True:
        ...
        result = bert_in.union(electra_in)
        result = result.union(roberta_in)
        result = list(result)
        result.sort()
